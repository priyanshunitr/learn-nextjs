...
export async function PUT(request, { params }) {
  await connectDB();
  const editJsonData = await request.json();
  const data = await db.collection("Sample.Collection").find().toArray();
  const { id } = await params;

  // const indexOfDataToBeChanged = data.findIndex((data) => id == data.id);

  const editedData = await db.collection("Sample.Collection").findByIdAndUpdate(id, editJsonData);

  return Response.json(editedData);
}
...
export async function DELETE(_, { params }) {
  await connectDB();
  const { api } = await params;
  const dataAfterDelete = await db.collection("Sample.Collection").deleteOne({ _id: api });
  return Response.json(dataAfterDelete);
}